...
export function NewListUsersRequest(req: Request): ListUsersRequest {
  ...
  if (filter) {
    if (filter.id) {
      filter.id = [...(filter.id instanceof Array ? filter.id.map((id)=> id as number) : [filter.id as number])];
    }
    ...
    if (filter.age) {
      filter.age = [...(filter.age instanceof Array ? filter.age : [filter.age as number])];
    }
    if (filter.role) {
      filter.role = [
